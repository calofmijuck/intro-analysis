...
\ex{7.6} Consider \(f_n(x) = nx(1-x^2)^n\) for \(x \in [0, 1]\). Note that
\begin{center}
    \(f_n(0) = 0\), \(f_n(1) = 0\).
\end{center}
...
\pf\footnote{고른수렴이 어디에서 쓰였는지 확인하는 것이 중요하다.}\\
...
\note{\(\ds \lim_{n \ra \infty} A_n = \lim_{t \ra x} f(t)\)} Let \(A = \ds \lim_{n \ra \infty} A_n\). We want to show that for all \(\epsilon > 0\),
\begin{center}
...
\thm{7.12} Suppose \(f: E \ra \C\) is continuous on \(E\) and \(f_n \uc f\) on \(E\). Then \(f\) is continuous on \(E\).
...
\thm{7.13} \note{Dini} Given a compact metric space \(K\), suppose that
\begin{enumerate}
