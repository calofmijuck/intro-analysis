...
\defn. Suppose \(f_n: E \ra \C\). We say that \(\seq{f_n}\) is \textbf{pointwise bounded} on \(E\) if the sequence \(\seq{f_n(x)}_{n=1}^\infty\) is bounded for all \(x \in E\). Or equivalently,
\begin{center}
...
Therefore \(\seq{f_{n_k^k}(x_i)}\) converges, and \(\seq{f_{n_k^k}(x)}\) converges for all \(x \in E\).
...
\defn{7.22} Let \((X, d)\) be a metric space, and \(E \subset X\). Let \(\scr{F}\) be a family of complex-valued functions on \(E\). We say that \(\scr{F}\) is \textbf{equicontinuous} on \(E\) if
\begin{center}
...
\thm{7.25} Let \(K\) be a compact set. Suppose \(f_n \in C(K)\). (Bounded and continuous) Suppose that \(\seq{f_n}_{n=1}^\infty\) is pointwise bounded and equicontinuous on \(K\). Then
\begin{enumerate}
